...
- `남은 항목 모두 넘기기`는 **요청 1회**(dismiss는 집합 add라 락·revision이 없다) → 로그 1건 →
...
레일은 `md`(768)이고 창 최소폭이 800이라 사실상 항상 보인다. 접히는 구간은 800~1023뿐이고,
그 구간에서 3열은 항목 폭을 300px대로 만든다 — 읽어야 할 것은 항목이다.
