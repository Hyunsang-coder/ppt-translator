...
| D-2 | 레이아웃은 **반응형 축소** (`lg` 미만 용어집 접기, `md` 미만 레일 접기 — 구현 시 조정) | 앱 기본 창 1100×800 / 최소 800 (`src-tauri/tauri.conf.json`), 디자인은 1280 기준 |
...
- 2문단 이상 → `applyReviewBlockEdit` 한 요청 (revision 1회 · 스냅샷 1개). `propagate_identical`은
  이 경로도 받는다 — 전파 여부가 편집 경로에 따라 달라지면 체크박스가 거짓말을 한다.
  **`partial_candidates`만 없다** (엔드포인트가 만들지 않는다). 명시적으로 편집한 문단은 다른
  문단의 전파가 덮지 않는다
- AI 재번역도 **항목 단위**다. 1문단은 proposal 경로, 병합 블록은
  `POST /review/block/retranslate` → 원문을 이어 붙여 한 번 번역하고 결과를 원문 길이 비율대로
  다시 줄로 나눈다(`_wrap_to_weights`, 공백이 있으면 공백에서 끊는다). 문단 하나만 다시 번역하면
  나머지 반쪽이 옛 문장으로 남는다 — 애초에 그래서 묶인 블록이다. 적용은 `/review/block`이라
  revision·되돌리기도 1회
...
**Phase 4·5 검증**: `npx tsc --noEmit` 통과 · `npm test` 38 passed · `npm run build` 성공.
- 부분 일치는 플로팅 시트를 버리고 **항목 슬롯을 한 번 차지하는 카드**가 된다 (`PartialMatchCard`).
  후보가 1건이면 체크박스 없이 `적용`/`건너뛰기`
- 완료 화면은 벽이 아니다: `처리한 항목 다시 보기`로 큐에 되돌아가고, 새 검출이 생기면 스스로 사라진다
- `남은 항목 모두 넘기기`는 **로그 1건** → `되돌리기` 한 번에 전부 복구 (dismiss는 집합 add라
  락·revision이 없다). 엔드포인트의 entries 상한이 2000건이라 `api-client`가 그 단위로 쪼개
  보내고 `changed`를 합친다 — 큰 덱은 검출이 2000건을 넘길 수 있다
...
**D-2 브레이크포인트 조정**: 계획의 `xl`(1280) 미만 용어집 접기를 그대로 쓰면 **기본 창 1100에서
용어집이 사라진다** — 기본 크기에서 기능을 잃는 건 반응형의 목적이 아니다. `lg`(1024)로 내렸다.
용어집 빠른 추가는 이 패널에만 있어서, 접기만 하면 **기능 자체가 사라진다** — `lg` 미만에서는
같은 패널을 오른쪽 서랍으로 겹쳐 띄우고(`용어집` 플로팅 버튼) 열은 접는다. 항목 폭은 그대로 두고
기능은 어느 폭에서도 남는다. 브라우저는 창 최소폭이 없으므로 이 폴백이 유일한 해법이다.
레일은 `md`(768)이라 사실상 항상 보인다.
