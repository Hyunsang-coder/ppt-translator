...
def _build_batch(
    paragraph_list: Sequence["ParagraphInfo"],
    start_idx: int,
    end_idx: int,
    ppt_context: str,
    glossary_terms: str,
    prepared_texts: Sequence[str] | None,
) -> Dict[str, object]:
    """Build one translation batch payload."""
    chunk = paragraph_list[start_idx:end_idx]
    lines = []
    for offset, paragraph in enumerate(chunk, start=1):
        if prepared_texts is not None:
            raw_text = prepared_texts[start_idx + offset - 1]
...
def chunk_paragraphs_by_tokens(
    ...
    """Split paragraphs using both item count and estimated prompt tokens.

    The estimate intentionally stays simple to avoid another tokenizer/runtime
    dependency. Single paragraphs that exceed the token limit are kept as one
    batch so the caller can still try translating them.
    """
    paragraph_list = list(paragraphs)
    ...
    for idx, paragraph in enumerate(paragraph_list):
        raw_text = (
            prepared_texts[idx]
            if prepared_texts is not None
            else paragraph.original_text
        )
        item_tokens = _approximate_tokens(clean_text(raw_text)) + 8
        would_exceed_items = current_items >= max_items
        would_exceed_tokens = (
            current_items > 0
    ...
    if start_idx < len(paragraph_list):
        batches.append(
            _build_batch(
                paragraph_list,
                start_idx,
